...
import { Calculator, Zap, Waves } from 'lucide-react';

// Constantes physiques fixes - Valeurs du TD LSPh411N (en dehors du composant)
const CONSTANTS = {
  c: 299792458, // m/s
...
export default function App() {
  ...
  const safeEvaluate = useCallback((expression) => {
    ...
    try {
      const sanitized = expression.replace(/[^0-9+\-*/().\se]/g, '');
  ...
  // Calcul automatique des champs avec dépendances optimisées
  useEffect(() => {
    const k = CONSTANTS.omega / CONSTANTS.c;
    setParameters({ k });

    if (inputMode === 'electric') {
      // Calculer B à partir de E
      const Ex = safeEvaluate(fieldEquations.Ex);
      const Ey = safeEvaluate(fieldEquations.Ey);
      const Ez = safeEvaluate(fieldEquations.Ez);

      let Bx = 0, By = 0, Bz = 0;

      if (propagationAxis === 'x') {
        // k = [k, 0, 0], B = k × E / ω
        Bx = 0;
        By = -k * Ez / CONSTANTS.omega;
        Bz = k * Ey / CONSTANTS.omega;
      } else if (propagationAxis === 'y') {
        // k = [0, k, 0], B = k × E / ω
        Bx = k * Ez / CONSTANTS.omega;
        By = 0;
        Bz = -k * Ex / CONSTANTS.omega;
      } else if (propagationAxis === 'z') {
        // k = [0, 0, k], B = k × E / ω
        Bx = -k * Ey / CONSTANTS.omega;
        By = k * Ex / CONSTANTS.omega;
        Bz = 0;
      }
      ...
      // Calculer E à partir de B
      const Bx = safeEvaluate(fieldEquations.Bx);
      const By = safeEvaluate(fieldEquations.By);
      const Bz = safeEvaluate(fieldEquations.Bz);
  ...
  }, [fieldEquations.Ex, fieldEquations.Ey, fieldEquations.Ez, fieldEquations.Bx, fieldEquations.By, fieldEquations.Bz, inputMode, propagationAxis, safeEvaluate]);
  ...
  const drawVector = useCallback((ctx, x, y, dx, dy, color, label, thickness = 3, showMagnitude = false) => {
  ...
  }, [inputMode, propagationAxis, fieldEquations.Ex, fieldEquations.Ey, fieldEquations.Ez, 
      fieldEquations.Bx, fieldEquations.By, fieldEquations.Bz, calculatedFields.Ex, calculatedFields.Ey, 
      calculatedFields.Ez, calculatedFields.Bx, calculatedFields.By, calculatedFields.Bz, 
      visualizationControls.magneticScale, visualizationControls.showMagneticField, 
      visualizationControls.showElectricField, visualizationControls.magneticThickness, 
      visualizationControls.electricThickness, drawVector, safeEvaluate]);

  // Rendu de la visualisation avec optimisation
  useEffect(() => {
    renderVisualization();
  }, [renderVisualization]);
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="space-y-6">
            <div className="bg-gray-800 rounded-lg p-6">
              ...
              <div className="mb-4">
                <label className="block text-sm font-medium mb-2">Mode d'entrée</label>
                <div className="flex space-x-4">
                  <label className="flex items-center">
                    <input
                      type="radio"
                  ...
                  {['Ex', 'Ey', 'Ez'].map(field => (
                    <div key={field} className="mb-2">
                      ...
                      <input
                        ...
                        onChange={(e) => setFieldEquations(prev => ({...prev, [field]: e.target.value}))}
                        disabled={inputMode === 'magnetic'}
                        className="w-full bg-gray-700 border border-gray-600 rounded px-2 py-1 text-sm font-mono disabled:opacity-50"
                      />
                    </div>
                  ))}
                </div>
          ...
          <div className="lg:col-span-2 bg-gray-800 rounded-lg p-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
              <div className="bg-gray-700 rounded p-3">
                <h4 className="font-medium mb-2 text-red-400">Champs calculés</h4>
                <div className="space-y-1 font-mono text-xs">
                  <div>Ex = {parseFloat(calculatedFields.Ex).toFixed(3)} V/m</div>
                  <div>Ey = {parseFloat(calculatedFields.Ey).toFixed(3)} V/m</div>
                  <div>Ez = {parseFloat(calculatedFields.Ez).toFixed(3)} V/m</div>
                  <div>Bx = {(parseFloat(calculatedFields.Bx)*1e9).toFixed(3)} nT</div>
                  <div>By = {(parseFloat(calculatedFields.By)*1e9).toFixed(3)} nT</div>
                  <div>Bz = {(parseFloat(calculatedFields.Bz)*1e9).toFixed(3)} nT</div>
                </div>
              </div>

              <div className="bg-gray-700 rounded p-3">
                <h4 className="font-medium mb-2 text-green-400">Vérifications physiques</h4>
                <div className="space-y-1 text-xs">
                  <div>Mode: {inputMode === 'electric' ? 'E⃗ → B⃗' : 'B⃗ → E⃗'}</div>
                  <div>Propagation: {propagationAxis.toUpperCase()}</div>
                  <div>|E| = {Math.sqrt(
                    Math.pow(parseFloat(calculatedFields.Ex), 2) +
                    Math.pow(parseFloat(calculatedFields.Ey), 2) +
                    Math.pow(parseFloat(calculatedFields.Ez), 2)
                  ).toFixed(2)} V/m</div>
                  <div>|B| = {(Math.sqrt(
                    Math.pow(parseFloat(calculatedFields.Bx), 2) +
                    Math.pow(parseFloat(calculatedFields.By), 2) +
                    Math.pow(parseFloat(calculatedFields.Bz), 2)
                  ) * 1e9).toFixed(2)} nT</div>
                </div>
              </div>
            </div>
          </div>
